...
async def download_track_history(url):
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            if response.status == 200:
                ...
                logging.error(f"Failed to download track history. Status code: {response.status}")
                return None

def parse_track_history(json_data):
    if json_data:
        try:
...
async def process_station(station_id):
    api_url = f"https://au.api.iheart.com/api/v3/live-meta/stream/{station_id}/trackHistory?limit=10000"
    track_history = await download_track_history(api_url)
    
    if track_history:
        track_ids = parse_track_history(track_history)
        return station_id, track_ids
    else:
        return station_id, []

async def save_to_json(station_id, track_ids):
    filename = f"trackhistory_{station_id}.json"
    data = {"station_id": station_id, "track_ids": track_ids}
    
    async with aiofiles.open(filename, "w") as file:
        await file.write(json.dumps(data, indent=4))

async def main(station_ids):
    tasks = [process_station(station_id) for station_id in station_ids]
    results = await asyncio.gather(*tasks)

    for station_id, track_ids in results:
        await save_to_json(station_id, track_ids)
        print(f"Saved track history for Station ID {station_id} to trackhistory_{station_id}.json")
